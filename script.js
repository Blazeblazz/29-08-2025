...
document.getElementById('quickOrderForm').addEventListener('submit', function(e) {
    ...
    // Redirect to unified thank you page
    if (selectedProduct.type === '3-pack') {
        window.location.href = 'thank-you.html?product=pack';
    } else {
        window.location.href = 'thank-you.html?product=tee';
    }
...
async function sendOrderToJSONBin(orderData) {
    try {
        ...
        // Update JSONBin with all orders
        ...
        if (response.ok) {
            console.log('Commande sauvegardée permanente:', orders.length, 'commandes totales');
        } else {
...
function sendOrderToGoogleSheets(orderData) {
    ...
    .then(data => console.log('Envoyé vers Google Sheets:', data))
